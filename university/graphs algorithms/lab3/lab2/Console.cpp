...
void Console::print_menu()
{
	...
	std::cout << "Press 18 to bfs\n";
	std::cout << "Press 19 to ford\n";
	std::cout << "Press 20 to exit\n";
}
...
void Console::print_bfs()
{
	...
	try
	{
		res = graph.bfs(end_node, start_node);
	}
...
void Console::print_bellman_ford()
{
	int start_node;
	int end_node;
	fseek(stdin, 0, SEEK_END);
	...
	try
	{
		res = graph.ford(start_node, end_node);
	...
	catch (const NegativeCycleException& nce)
	{
		std::cout << nce.what() << "\n";
	}
	catch (const std::exception& ex)
