...
void Tests::test_delete_repository()
{
	...
	assert(repository->get_all().size() == 0);
	try
	{
...
void Tests::test_add_element_in_shopping_basket()
{
	...
	assert(user_repository->get_all().size() == 1);
	assert(service->get_elements_in_shopping_basket().size() == 1);
	...
	assert(service->get_elements_in_shopping_basket().size() == 0);
