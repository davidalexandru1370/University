...
void Console::print_trenches(int size = 0)
{
	...
	Vector<Trench> list = service->get_all();
	for (size_t index = 0; index < list.get_length(); index++)
	{
...
{
	Vector<Trench> list = service->get_elements_in_shopping_basket();
	int sum = 0;
	for (size_t index = 0; index < list.get_length(); index++)
	{
		Trench trench = list[index];
